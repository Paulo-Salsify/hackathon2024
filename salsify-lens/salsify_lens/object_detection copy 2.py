...
class VideoObjectDetection:
    ...
    def start_video_analysis(self) -> None:
        ...
        while True:
            ...
            prediction = self.object_detection_model.predict(processed_frame)

            # Apply thresholding for prediction
            detection_result = self.apply_threshold(prediction)
    ...
    def apply_threshold(self, prediction: np.ndarray) -> int:
        """
        Apply the threshold to the model prediction.
        """
        return (prediction > THRESHOLD).astype("int32")[0][0]
